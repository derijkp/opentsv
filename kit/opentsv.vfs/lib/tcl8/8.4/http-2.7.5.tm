...
proc http::geturl {url args} {
    ...
    # We cannot handle chunked encodings with -handler or -channel at the
    # moment, so force HTTP/1.0 until we can manage this.
    if {[info exists state(-handler)] || [info exists state(-channel)]} {
